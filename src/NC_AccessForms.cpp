...
void ReadMatrices(std::string const& FileName, size_t& index, size_t n_short) {
  ...
  //
  size_t dim = 45;
  netCDF::NcVar var2 = dataFileI.getVar("minQ");
  ...
  for (size_t i=0; i<number_perfect; i++) {
    if (V_minQ[i] == n_short) {
      ...
      MyMatrix<mpz_class> M = ReadMatrix(V_form);
      std::string MatFile = "Matrix_" + std::to_string(index);
...
int main(int argc, char *argv[]) {
  try {
    ...
    size_t n_short = ParseScalar<int>(n_short_str);
    //
    // Now reading the
    //
    size_t index = 0;
